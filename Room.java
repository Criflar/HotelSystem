...
public class Room {
    ...
    public void setDaysBooked (boolean status, int startingDay, int numDays) {
        int i;

        for (i = startingDay; i < startingDay + numDays; i++) { // Books from start to last day.
    ...
    public boolean checkAvailability (int checkIn, int checkOut) {
      ...
      //checkIn: 1-30, checkOut: 2-31
        if (checkIn < 1 || checkIn > 30 || checkOut < 2 || checkOut > 31 || checkOut < checkIn) {
            return false;
        }
